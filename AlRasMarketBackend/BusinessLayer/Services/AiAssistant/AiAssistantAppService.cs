    ...
    ILogger<AiAssistantAppService> logger) : IAiAssistantAppService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var system =
            ...
            - find_cheapest_product: MUST call this when the user asks أرخص / cheapest. Pass only the product name (هيل, cardamom) — never the whole sentence. If they said "أرخص منتج" without naming one, omit product_name. UNIT PRICE: tool.unitPrice is for ONE unitName (e.g. 160000 USD per Ton). availableQuantity is stock only. NEVER say the price is for the whole stock. NEVER multiply. Spoken: «السعر 160000 للدولار للطن الواحد، والكمية المتوفرة 50 طن». Cards appear under the reply. If showPrice=false / priceAvailable=false, do NOT speak any amount — say ask for price (اطلب السعر).
            - find_most_expensive_product: same unit-price rules as find_cheapest_product (أغلى / most expensive). Never multiply price by quantity. Same hidden-price rule.
            - search_products: search public ads by product name/type. MUST call this when they want to see ads/cards without asking cheap/expensive. Cards appear in chat; summarize, do not paste a long catalog. Never invent prices when showPrice=false.
            ...
            When talking about public catalog results (search / cheapest / most expensive), describe ONLY the product (name, price only when showPrice=true, quantity, unit, availability).
            If a tool payload includes seller/company fields, ignore them completely and never repeat them to the user.
            CRITICAL hidden catalog price: if any tool result has showPrice=false or priceAvailable=false (or null price), never invent, guess, or speak a numeric price — tell the user to ask for price (اطلب السعر). Seller-owned tools (list_my_ads / update_ad_price_quantity) may still show the owner's own prices.
