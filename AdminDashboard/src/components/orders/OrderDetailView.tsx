...
import { formatAdAmount, formatPackagingLabel, formatPriceTypeLabel, productTypeBadgeClass, resolveOrderChannelTypeKey, resolveOrderChannelTypeName } from '../../utils/adsDisplay'
import {
  formatOrderAmount,
...
export default function OrderDetailView({
  ...
  const imageUrl = resolveAssetUrl(
    order.primaryImagePath ?? order.images[0]?.path ?? order.productImagePaths?.[0],
  )
  ...
  const isRequestOrder = order.productTypeName.trim().toLowerCase().includes('request')
  const isBookingOrder =
    order.productTypeName.trim().toLowerCase().includes('booking') ||
    order.productTypeName.trim().includes('حجز')
  const typeKey = resolveOrderChannelTypeKey(order)
  const typeName = resolveOrderChannelTypeName(order, locale)
  const isOffersOrder =
    typeKey === 'offers' ||
    typeKey === 'عروض' ||
    typeKey.includes('offer')
  ...
  const priceTypeLabel = formatPriceTypeLabel(
    order.requestTypeName,
    t,
    order.requestTypeId,
  )
  const negotiableLabel =
    order.negotiable === true
      ? t('ads.negotiableYes')
      : order.negotiable === false
        ? t('ads.negotiableNo')
        : t('ads.negotiableUnknown')
  const packagingLabel =
    formatPackagingLabel(order.packaging, order.packagingDetails, t, locale) || '—'
  const offerDurationLabel = order.offerDuration?.trim() || '—'
    ...
    <div className="order-print-root space-y-5 print:space-y-3">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 print:hidden">
        <div className="flex min-w-0 flex-1 items-start gap-3 text-start">
          {imageUrl ? (
            ...
            <span className="admin-surface-muted flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-xs">
              —
            </span>
          )}
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${status.className}`}
              >
      ...
      <div
        className={`grid gap-3 print:hidden sm:grid-cols-2 ${
          isOffersOrder ? 'xl:grid-cols-[1fr_1fr_1fr_1.35fr_0.85fr]' : 'xl:grid-cols-5'
        }`}
      >
        <MetricCard
        ...
        />
        <MetricCard
          label={isRequestOrder ? t('orders.offeredQuantity') : t('orders.orderedQuantity')}
      ...
      {(canSetCustomRequestStatus || canMarkReceived) && (
        <section className="admin-card rounded-2xl p-5 shadow-sm print:hidden">
          <h2 className="admin-text text-start text-sm font-bold">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(260px,300px)]">
        <div className="space-y-4">
          ...
          >
                  <section className="admin-card rounded-2xl p-5 shadow-sm sm:p-6">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                      ...
                      {!showProductTypeField ? null : (
                        <IconInfoField
                          ...
                          className={
                            isOffersOrder
                              ? 'rounded-xl bg-[#FFF4E5]/80 px-2.5 py-2 ring-1 ring-[#FFD4A8]/70'
                              : ''
                          }
                          value={
                            <span
                              className={`inline-flex rounded-md px-2 py-0.5 text-[10px] font-bold ${productTypeBadgeClass(typeKey)}`}
                            >
                              {typeName}
                            </span>
                          }
                      ...
                      {isOffersOrder ? (
                        <>
                          <IconInfoField
                            ...
                            value={
                              <span className="inline-flex rounded-md bg-violet-50 px-2 py-0.5 text-[10px] font-bold text-violet-700 ring-1 ring-violet-100">
                                {priceTypeLabel}
                              </span>
                            }
                          />
                          ...
                          />
                          <IconInfoField
                            label={t('ads.offerDuration')}
                            icon={InfoFieldIcons.calendar}
                            iconClass="bg-amber-50 text-amber-600"
                            value={offerDurationLabel}
                          />
                        </>
                      ) : null}
                      <IconInfoField
                        ...
                        iconClass="bg-amber-50 text-amber-600"
                        value={order.productDescription?.trim() || t('ads.noDescription')}
                      />
                      {isRequestOrder ? (
                        <IconInfoField
                          label={t('orders.offeredQuantity')}
                          icon={InfoFieldIcons.package}
                          iconClass="bg-orange-50 text-orange-500"
                    ...
                    {productImagePaths.length > 0 ? (
                      <div className="mt-5 border-t border-slate-100 pt-4">
                        <p className="admin-text mb-2.5 text-start text-xs font-bold">
                          {t('orders.productImages')}
                        </p>
                        <CompactMediaStrip
                          paths={productImagePaths}
                          sizeClassName="h-20 w-28 sm:h-24 sm:w-32"
                        />
                      </div>
                    ) : null}
                  </section>
                  ...
                  <section className="admin-card rounded-2xl p-5 shadow-sm sm:p-6">
                    <IconInfoSectionTitle
                      title={t('orders.orderInformation')}
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
                        <div className="mb-4 flex items-start gap-2.5">
                          ...
                          <div className="min-w-0 text-start">
                            ...
                            <p className="mt-0.5 text-[11px] font-medium text-emerald-700/80 dark:text-emerald-300/70">
                              {t('orders.companyEarningsHint')}
                            </p>
                          </div>
                        </div>
                      ...
                      {/* Supplier pricing */}
                      <div className="rounded-2xl border border-sky-200 bg-sky-50/60 p-4 dark:border-sky-900/50 dark:bg-sky-950/20">
                        <div className="mb-4 flex items-start gap-2.5">
                          ...
                          <div className="min-w-0 text-start">
                            ...
                            <p className="mt-0.5 text-[11px] font-medium text-sky-700/80 dark:text-sky-300/70">
                              {t('orders.supplierPricingHint')}
                            </p>
                          </div>
                        </div>
                        <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <div className="mt-5 space-y-4 border-t border-slate-100 pt-5">
                        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                          ...
                          <IconInfoField
                            ...
                            value={
                              order.refundedAtUtc
                                ? formatDetailDate(order.refundedAtUtc, locale)
                                : '—'
                            }
                          />
                          <IconInfoField
        ...
        <aside className="space-y-4 print:hidden">
          <SidebarCard
            title={isRequestOrder ? t('reqsOffers.requestOwnerInfo') : t('orders.customerInfo')}
            icon={Icons.user}
          >
          ...
          <SidebarCard
            title={isRequestOrder ? t('reqsOffers.offeringSupplierInfo') : t('orders.supplierInfo')}
            icon={Icons.user}
          >
            <div className="space-y-3 text-start">
              <div className="flex items-center gap-3">
