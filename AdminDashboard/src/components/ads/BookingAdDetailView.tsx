...
/**
 * Layout matches the mock exactly (no page scroll):
 *
 * [ Hero                    ] [ Ad Status | Supplier ]
 * [ Basic Information       ] [ Video ]
 *                             [ Admin Notes ]
 * [ Additional Information  ] [ Shipping & Route ]
 * [ Quick Overview — full width ]
 */
export default function BookingAdDetailView({
  product,
  lookups,
  isSaving,
  isApproving,
  ...
  const priceTypeLabel = formatBookingPriceTypeLabel(
    product.bookingPriceTypeName,
    product.bookingPriceTypeId,
  )
  const incotermLabel = priceTypeLabel
  const negotiableShort =
  ...
  return (
    <div
      className="booking-ad-fit origin-top space-y-3.5"
    >
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5 text-start">
      ...
      {/* ROW 1: Hero | Status + Supplier side-by-side */}
      <div className="grid grid-cols-1 gap-3.5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,1fr)]">
        <Card>
          <div className="grid gap-4 p-4 sm:grid-cols-[170px_minmax(0,1fr)]">
            <div
              className="flex min-h-[160px] flex-col items-center justify-center gap-2.5 rounded-xl border border-dashed px-3 py-4 text-center"
            ...
            >
              ...
              <OutlineBtn disabled={isBusy} onClick={() => fileInputRef.current?.click()}>
                {I.upload}
                {isUploading ? '…' : t('ads.uploadImage')}
              </OutlineBtn>
              {mainUrl && mainImage ? (
                <button
                  type="button"
              ...
              {mainUrl && mainImage && onReplaceImage ? (
                <button
                  type="button"
              ...
              {mainUrl && mainImage ? (
                <button
                  ...
                  style={{ color: C.blue }}
                  onClick={() =>
                    void downloadAsset(mainUrl, filenameFromAssetPath(mainImage.path)).catch(() =>
            ...
            <div className="min-w-0 text-start">
              ...
              <div className="mt-3 grid gap-x-4 gap-y-2.5 sm:grid-cols-3">
                <Labeled label={t('ads.price')}>
                  {priceWithUnit}
                </Labeled>
                <Labeled label={t('ads.quantity')}>{quantityLabel}</Labeled>
                <Labeled label={t('ads.packagingType')}>{packagingLabel}</Labeled>
                <div className="rounded-md px-2.5 py-1.5" style={{ backgroundColor: C.purpleBg }}>
                  <p className="text-[11px] font-bold" style={{ color: C.navy }}>
                    {t('ads.requestFulfillment')}
                  </p>
                  <p className="text-[13px] font-normal" style={{ color: '#64748B' }}>
                    {priceTypeLabel}
                  </p>
                </div>
                <Labeled label={t('ads.negotiable')}>{negotiableShort}</Labeled>
      ...
      {/* ROW 2: Basic Info | Video + Admin Notes */}
      <div className="grid grid-cols-1 gap-3.5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,1fr)]">
        <Card>
          <SectionHead
                    ...
                    {specs.length > 0 ? (
                      <ul className="space-y-1">
                        {specs.map((item, index) =>
                          typeof item === 'string' ? (
                            <li key={`${item}-${index}`} className="text-[11px] font-normal whitespace-pre-wrap" style={{ color: '#64748B' }}>
                              {item}
                            </li>
                          ) : (
                            <li key={`${item.label}-${index}`} className="flex justify-between gap-2 text-[11px]">
                              <span className="font-bold" style={{ color: C.navy }}>{item.label}</span>
                              <span className="font-normal" style={{ color: '#64748B' }}>
                                {item.value}
                              </span>
                            </li>
      ...
      {/* ROW 3: Additional Info | Shipping */}
      <div className="grid grid-cols-1 gap-3.5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,1fr)]">
        <Card>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
          <OverviewTile icon={I.money} label={t('ads.price')} value={priceWithUnit} bg={C.blueSoft} fg={C.blue} />
          <OverviewTile icon={I.truck} label={t('ads.quantity')} value={quantityLabel} bg={C.blueSoft} fg={C.blue} />
          <OverviewTile
            icon={I.package}
          ...
          <OverviewTile
            ...
            fg={C.orange}
          />
          <OverviewTile icon={I.tag} label={t('ads.adType')} value={typeName} bg={C.blueSoft} fg={C.blue} />
        </div>
      </div>
